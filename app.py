...
sf_home_data = load_data(sf_home_url)
sf_away_data = load_data(sf_away_url)
...
sf_home_data = normalize_columns(sf_home_data)
sf_away_data = normalize_columns(sf_away_data)

# Colunas principais para identificação
home_team_col = "Equipe"
away_team_col = "Equipe_Fora"
overall_stats_col = "Equipe"

# Listar as colunas necessárias
...
# Prosseguir se estiver tudo certo
if not (missing_columns_home or missing_columns_away or missing_columns_away_fav or missing_columns_overall):
    # Filtros
    equipe_home = st.sidebar.selectbox("Selecione a equipe Home:", sorted(home_data[home_team_col].unique()))
    equipe_away = st.sidebar.selectbox("Selecione a equipe Away:", sorted(away_data[away_team_col].unique()))
    equipe_away_fav = st.sidebar.selectbox("Selecione a equipe Away (Favorito):", sorted(away_fav_data[away_team_col].unique()))
    ...
    gf_avg_home_min, gf_avg_home_max = st.sidebar.slider("Média de Gols (Home)", float(home_data["GF_AVG_Home"].min()), float(home_data["GF_AVG_Home"].max()), (home_data["GF_AVG_Home"].min(), home_data["GF_AVG_Home"].max()))
    gf_avg_away_min, gf_avg_away_max = st.sidebar.slider("Média de Gols (Away)", float(away_data["GF_AVG_Away"].min()), float(away_data["GF_AVG_Away"].max()), (away_data["GF_AVG_Away"].min(), away_data["GF_AVG_Away"].max()))
    ...
    away_fav_filtered_team = away_fav_data[away_fav_data[away_team_col] == equipe_away_fav][required_columns_away]
    overall_filtered_team = overall_stats_data[overall_stats_data[overall_stats_col] == equipe_home][required_columns_overall]

    # Dados scored first
    sf_home_row = sf_home_data[sf_home_data["Equipe"] == equipe_home]
    sf_away_row = sf_away_data[sf_away_data["Equipe"] == equipe_away]
    ...
    if not sf_home_row.empty:
        st.markdown(f"🔵 **{equipe_home} marcou o primeiro gol em {sf_home_row['Perc.'].values[0]} das partidas em casa.**")

    st.subheader("Away (Zebra)")
    st.dataframe(away_filtered_team.reset_index(drop=True))
    if not sf_away_row.empty:
        st.markdown(f"🔴 **{equipe_away} marcou o primeiro gol em {sf_away_row['Perc.'].values[0]} das partidas fora.**")

    st.subheader("Away (Favorito)")
    st.dataframe(away_fav_filtered_team.reset_index(drop=True))

    st.subheader("1x2 (Home)")
    st.dataframe(home_data[(home_data["PIH"] >= pih_min) & (home_data["PIH"] <= pih_max)][[home_team_col] + required_columns_home].reset_index(drop=True))

    st.subheader("1x2 (Away)")
    st.dataframe(away_data[(away_data["PIA"] >= pia_min) & (away_data["PIA"] <= pia_max)][[away_team_col] + required_columns_away].reset_index(drop=True))

    st.subheader("HA +0.25 (Home)")
    st.dataframe(home_data[(home_data["PIH_HA"] >= piha_min) & (home_data["PIH_HA"] <= piha_max)][[home_team_col] + required_columns_home].reset_index(drop=True))

    st.subheader("HA +0.25 (Away)")
    st.dataframe(away_data[(away_data["PIA_HA"] >= piah_min) & (away_data["PIA_HA"] <= piah_max)][[away_team_col] + required_columns_away].reset_index(drop=True))

    st.subheader("Média de Gols (Casa)")
    st.dataframe(home_data[(home_data["GF_AVG_Home"] >= gf_avg_home_min) & (home_data["GF_AVG_Home"] <= gf_avg_home_max)][[home_team_col] + required_columns_home].reset_index(drop=True))

    st.subheader("Média de Gols (Away)")
    st.dataframe(away_data[(away_data["GF_AVG_Away"] >= gf_avg_away_min) & (away_data["GF_AVG_Away"] <= gf_avg_away_max)][[away_team_col] + required_columns_away].reset_index(drop=True))
