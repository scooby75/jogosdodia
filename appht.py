...
with tabs[7]:
    ...
    # 6. Construção da Tabela de Comparação
    ...
    # Comparativo visual (opcional)
    fig = go.Figure(data=[
        go.Bar(name=equipe_home, x=synthese_df['Indicador'], y=[float(i.replace('%', '').replace('-', '0')) for i in synthese_df[equipe_home]]),
        go.Bar(name=equipe_away, x=synthese_df['Indicador'], y=[float(i.replace('%', '').replace('-', '0')) for i in synthese_df[equipe_away]])
    ])
